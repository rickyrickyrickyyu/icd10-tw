...
import DermCommon from './components/DermCommon.jsx';
...
export default function App() {
  ...
  else if (core.error && !core.meta) body = <p className="p-6 text-red-700">資料載入失敗：{core.error}</p>;
  ...
  else if (route.view === 'derm') body = <DermCommon key={rk} core={core} group={route.key} />;
  else if (route.view === 'batch') body = <Batch core={core} />;
      ...
      <header className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-slate-200">
        ...
        <div className="max-w-5xl mx-auto px-3 sm:px-4 py-2 flex flex-col gap-2">
          <div className="flex items-start justify-between gap-2">
            <a href="#/" className="font-semibold text-brand-900 whitespace-nowrap mt-0.5">ICD-10 健保診斷碼查詢</a>
            <nav className="flex flex-wrap justify-end gap-x-3 gap-y-0.5 text-sm text-slate-600 mt-3 sm:mt-2.5">
              {NAV.map(([h, label]) => <a key={h} href={h} className="hover:text-brand-700 whitespace-nowrap">{label}</a>)}
            </nav>
          </div>
          <SearchBar core={core} initial={route.view === 'search' ? route.q : ''} />
          <div className="flex flex-wrap items-center gap-1 text-xs" role="tablist" aria-label="查詢範圍">
            {SCOPES.map(([k, label]) => (
              <button
                key={k}
                type="button"
            ...
            {core.building && <span className="ml-2 text-slate-500" data-testid="building">載入全部診斷中…（首次約數秒，之後會快取）</span>}
            {core.buildingPcs && <span className="ml-2 text-slate-500">載入處置 PCS 中…</span>}
            {core.error && core.meta && <span className="ml-2 text-red-700">資料載入失敗：{core.error}（請重新整理）</span>}
          </div>
        </div>
      </header>
