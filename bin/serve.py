...
用法：
    python3 bin/serve.py [--port 5193] [--dir dist] [--prefix /icd10-tw/]

為什麼不用 `pnpm dev`：門診場景要的是「秒開、不需 node_modules、不重編譯」。
dist/ 已經是建置好的產物，用 stdlib 的 http.server 起來只要幾十毫秒。

為什麼要 prefix：vite.config.js 的 base 是 /icd10-tw/（GitHub Pages 子路徑），
...
def main() -> int:
    ...
    ap.add_argument("--port", type=int, default=5193)
    ap.add_argument("--dir", default="dist")
    ap.add_argument("--prefix", default="/icd10-tw/")
    args = ap.parse_args()
    ...
    port = args.port
    for _ in range(6):                      # 5193–5198（避開 nhi-drug-rules 的 5183–5188）
        if port_free(port):
            break
        port += 1
    else:
        print("❌ 5193–5198 都被佔用", file=sys.stderr)
        return 1
