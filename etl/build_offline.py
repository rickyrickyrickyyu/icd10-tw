...
功能
----
- 中文、英文、俗稱、縮寫、錯字、代碼（L400＝L40.0）、2014 舊碼、ICD-9 都能查
- 每筆結果可直接「複製」代碼；複製時會提醒須另加編碼／須先編的規定
- 「批次查碼」：貼上多行診斷，一次查出每行的代碼（完全在本機處理，不用 AI、不上傳）
- 「皮膚科常用」清單、常用碼匯出／匯入、代碼頁可依側別與就醫階段篩選下層碼
