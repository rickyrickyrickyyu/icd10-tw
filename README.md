...
- 入口詞 CM 239,860 條、PCS 71,805 條（15 個來源，見[詞彙層](#詞彙層入口詞來源)）
...
| Wikidata 標籤／別名（簡體以台灣語料字集過濾） | 59,089 | 0.70／0.60 |
...
| **v10** | **俗稱 42＋縮寫 38 條經醫師全部核可**（目前 baseline） | **98.2／99.6** | **95.6／100** | 73.6／86.4 | 66.1／77.1 | 95.5／99.5 | 8.4 ms |
...
- Wikidata `zh` 語系混簡體 → 每個字都要出現在台灣醫學語料字集才收（擋掉 3,695 條）。
...
- **public/data 進版控**（沿用 nhi-drug-rules：CI 與本機讀同一份、可回溯）。代價是每次資料更新都會重寫
  cm.json（14.5 MB）、pcs.json（11.7 MB）、vocab_cm.json（9.7 MB），git 歷史每次約增加數十 MB（壓縮後較少）。
